...
const Lawyers = () => {
  ...
  return <Layout title="Meet Our Lawyers - Gibb Lawyers" description="We invest in our people to be better stakeholders for our Client-Partners. Our lawyers are vastly experienced in their array of expertise.">
      {/* Header Section */}
      ...
      <section className="py-12 sm:py-16 lg:py-24 xl:py-32">
        <div className="container mx-auto px-3 sm:px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 lg:gap-10">
            {lawyers.map((lawyer, index) => <a key={index} href={lawyer.url} target="_blank" rel="noopener noreferrer" className="group block">
                <Card className="h-full hover:shadow-elegant hover:border-primary/20 transition-all duration-500 border border-border/50 overflow-hidden">
                  <div className="aspect-[3/4] relative overflow-hidden">
                    <img src={lawyer.image} alt={`${lawyer.name} - ${lawyer.position}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading={index < 6 ? "eager" : "lazy"} />
